...
import '../screens/history_orders_screen.dart'; // 历史订单页面
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  final List<TextEditingController> _unitControllers = []; // 新增单位控制器
  ...
  void _addItemRow() {
    setState(() {
      ...
      _unitControllers.add(TextEditingController()); // 新增单位输入框
    });
  }
  ...
  void _removeItemRow(int index) {
    setState(() {
      ...
      _unitControllers.removeAt(index); // 删除对应的单位控制器
    });
  }
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      for (int i = 0; i < _itemControllers.length; i++) {
        ...
        final unit = _unitControllers[i].text; // 获取单位
        final quantity = (quantityText.isNotEmpty && int.tryParse(quantityText) != null) ? int.parse(quantityText) : 0;
      ...
      _unitControllers.clear(); // 清除单位控制器
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView( // 添加此行以支持滚动
